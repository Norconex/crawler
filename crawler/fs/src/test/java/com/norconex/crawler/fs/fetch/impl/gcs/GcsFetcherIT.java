...
class GcsFetcherIT {
    ...
    void testFetchFileObject() throws IOException {
        ...
        var fileName = mock(CloudStoragePath.class);
        ...
        when(fileName.toString()).thenReturn("bye.txt");
        ...
        try (MockedStatic<Files> files = mockStatic(Files.class)) {
            ...
            assertThat(FsTestUtil.toString(doc.getInputStream()))
                    .contains("Bye World!");
    ...
    void testFetchChildPaths() throws IOException {
        ...
        when(file.toUri()).thenReturn(URI.create("gs://bucket/bye.txt"));
        ...
        try (MockedStatic<Files> files = mockStatic(Files.class)) {
            files.when(() -> Files.newDirectoryStream(root)).thenReturn(stream);
            ...
            var response = (FolderPathsFetchResponse) fetcher.fetch(
                    new FolderPathsFetchRequest(new Doc("gs://bucket")));
